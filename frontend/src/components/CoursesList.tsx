        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {courses.map((item) => (
            <div key={item.id} className="flex flex-col group">
              <CourseCard course={item} onClick={() => handleNavigateToDetail(item.id)} />

              {/* Phần action buttons giữ nguyên */}
              <div className="flex items-center justify-between mt-3 px-1">
                <div className="text-sm text-gray-500">{item.totalLessons ?? 0} Materials</div>
                <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                  {/* ... (Giữ nguyên các nút View/Edit/Delete cũ của bạn) ... */}
                  <Button size="small" onClick={() => handleNavigateToDetail(item.id)}>
                    View
                  </Button>
