...
export class UserRepository implements IUserRepository{
    ...
    public async save(
        ...
        for (const [key, value] of Object.entries(specificUserDetail!)){
            if (value !== undefined)
                (specificUserDetail as any)[key] = value
            else 
                (specificUserDetail as any)[key] = null
        }
        for (const [key, value] of Object.entries(userDetail)){
            if (value !== undefined)
                (userDetail as any)[key] = value
            else 
                (userDetail as any)[key] = null
        }
        ...
        if (result === null){
            await this.db.any(
                'INSERT INTO "user"(${column_names:name}) VALUES (${values:list})',
                {
            ...
            if (user.role === Role.STUDENT){
                await this.db.any(
                    'INSERT INTO student(${column_names:name}) VALUES (${values:list})',
                    {
                ...
                await this.db.any(
                    'INSERT INTO instructor(${column_names:name}) VALUES (${values:list})',
                    {
                        column_names: Object.keys(specificUserDetail!),
                        values: Object.values(specificUserDetail!)
                    }
